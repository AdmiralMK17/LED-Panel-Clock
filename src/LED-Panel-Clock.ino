...
void setup() {
  ...
  //P.displayZoneText(0, "rhyrt", PA_CENTER, 25, 5000, PA_PRINT, PA_NO_EFFECT);
  //P.displayAnimate();
  ...
  if ( (WIFI_connected != WL_CONNECTED) or !CFG_saved) {
    ...
    config.ssid = "WiFi-Часы-v0.1";       // SSID of access point
  ...
  if (P.getZoneStatus(0))
  {
    switch (cycle)
    {
      ...
      case 1:
        ...
        //P.displayZoneText(0, buf, PA_CENTER, 25, 5000, catalog[rnd].effect, catalog[rnd].effect);
        Text =  " " + h + ":" + m + " ";
        utf8rus(Text).toCharArray(buf, 256);
        P.displayZoneText(0, buf, PA_CENTER, 25, 5000, PA_SCROLL_LEFT,PA_NO_EFFECT);
        cycle++;
        break;
      case 2:
        Text = L_today + " " + wd + " " + d + " " + mon + " " + y;
        utf8rus(Text).toCharArray(buf, 256);
        P.displayZoneText(0, buf, PA_LEFT, 25, 5000,  PA_SCROLL_LEFT, PA_CLOSING);
        cycle++;
        break;
      case 3:
        rnd = random(0, ARRAY_SIZE(catalog));
        Text = " " + h + ":" + m + " ";
        utf8rus(Text).toCharArray(buf, 256);
        P.displayZoneText(0, buf, PA_CENTER, 35, 5000, PA_SCROLL_LEFT, PA_NO_EFFECT);
        cycle++;
        break;
      case 4:
        rnd = random(0, ARRAY_SIZE(catalog));
        ...
        utf8rus(Text).toCharArray(buf, 256);
        P.displayZoneText(0, buf, PA_LEFT, 35, 5000,  PA_SCROLL_LEFT, PA_CLOSING);
        cycle++;
        break;
      case 5:
        cycle++;
...
void displayInfo() {
  if (P.displayAnimate()) {
    utf8rus(Text).toCharArray(buf, 256);
    P.displayZoneText(0, buf, PA_CENTER, catalog[rnd].speed, 5000, catalog[rnd].effect, catalog[rnd].effect);
...
void scrollText() {
  if  (P.displayAnimate()) {
    ...
    P.displayZoneText(0, buf, PA_LEFT, 25, 5000, PA_SCROLL_LEFT, PA_SCROLL_LEFT);
